...
router.post('/webhook', async (req, res) => {
  try {
    ...
    // Log incoming message in ChatHistory
    ...
    // 1. Detect intent and context (very basic keyword-based for demo)
    let botResponse = "Hi! I'm your RealYAI assistant. How can I help you with real estate today?";
    let leadStatus: 'HIGH' | 'MEDIUM' | 'NOT_QUALIFIED' | undefined = undefined;
    let followUpActivity: string | undefined = undefined;
    let contactInfo: string | undefined = undefined;
    let phoneNumber: string | undefined = undefined;
    let leadData = {};

    // Helper: simple contact validation
    function isValidContact(text: string) {
    ...
    // Use case 1: Office space inquiry
    ...
    // Use case: Market trends & lead magnet
    ...
    // Use case: Lead magnet download, no contact
    ...
    // Use case: Useless/time-pass user
    ...
    // --- Lead and FollowUp DB logic ---
    // Only create/update lead if intent is not useless/time-pass
    if (leadStatus && leadStatus !== 'NOT_QUALIFIED') {
      // Find existing lead by telegramUserId
    ...
    // Log bot response in ChatHistory
    await prisma.chatHistory.create({
      data: {
        telegramUserId,
        message: userText,
